        ...
        <div class="flex-1">
            ...
            <section class="hero">
                <div class="px-4 md:px-8 max-w-4xl">
                    ...
                    <a href="#opportunityCards"
                        class="btn-primary px-6 py-3 rounded-full font-semibold text-lg inline-block">
                ...
                <div class="mb-12">
                    ...
                    <div class="tabs-container">
                        <div class="tabs space-x-1">
                            <button onclick="filterOpportunities('all')"
                                class="tab tab-active px-6 py-3 font-medium transition-colors duration-200">All</button>
                            <button onclick="filterOpportunities('training')"
                                class="tab px-6 py-3 font-medium text-gray-600 hover:text-gray-900 transition-colors duration-200">Training</button>
                            <button onclick="filterOpportunities('volunteer')"
                                class="tab px-6 py-3 font-medium text-gray-600 hover:text-gray-900 transition-colors duration-200">Volunteering</button>
                            <button onclick="filterOpportunities('event')"
                                class="tab px-6 py-3 font-medium text-gray-600 hover:text-gray-900 transition-colors duration-200">Events</button>
                        </div>
                    </div>
                </div>
                    ...
                    @foreach ($opportunities as $opportunity)
                        <div class="card opportunity {{ strtolower($opportunity->type) }}">
                            @if ($opportunity->photo)
                                <img src="{{ route('opportunity.photo', basename($opportunity->photo)) }}"
                                    alt="Digital Marketing Workshop" class="card-image">
                            @else
                                —
                            @endif
                            <div class="card-content">
                                <div class="flex justify-between items-start mb-4">
                                    ...
                                    <span
                                        class="badge badge-{{ strtolower($opportunity->type) }}">{{ $opportunity->type }}</span>
                                </div>
                            ...
                            <div class="card-button">
                                @csrf
                                <button
                                    onclick="openApplicationModal('{{ $opportunity->title }}', '{{ $opportunity->type }}')"
                                    class="btn-primary w-full py-2 rounded-lg font-medium">
                                    {{ $opportunity->type == 'event' ? 'Register Now ' : 'Apply Now' }}</button>
                            </div>
                    ...
                    <form id="applicationForm" method="POST" action="{{ route('applications.store') }}">
                        ...
                        <div class="mb-4">
                            <label for="opportunityName"
                                class="block text-gray-700 font-medium mb-2">Opportunity</label>
                            <input type="text" id="opportunityName"
                                class="w-full px-4 py-2 border rounded-lg bg-gray-100"name="opportunity_title"
                                readonly>
                        </div>
                        ...
                        <div class="flex justify-end space-x-4">
                            <button type="button" onclick="closeModal()"
                                class="btn-secondary px-6 py-2 rounded-lg font-medium">Cancel</button>
                            <button type="submit" class="btn-primary px-6 py-2 rounded-lg font-medium">Submit
                                Application</button>
                        </div>
                    </form>
                </div>
