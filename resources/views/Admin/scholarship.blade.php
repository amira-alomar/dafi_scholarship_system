    ...
    @foreach ($scholarships as $scholarship)
      <div class="scholarship-card "id="scholarship-{{ $scholarship->scholarshipID }}">
        <div class="scholarship-header">
          ...
          <div class="button-group">
            ...
            <form action="{{ route('scholarships.destroy', $scholarship->scholarshipID) }}#scholarship-{{ $scholarship->scholarshipID }}" method="POST" style="display:inline">
              @csrf @method('DELETE')
              <button class="delete-btn" onclick="return confirm('Are you sure you want to delete it?')">Delete</button>
            </form>
          </div>
        </div>
        ...
        <div class="details-sections">
          ...
          <div class="relationship-section">
            ...
            <ul class="relationship-list">
              @foreach ($scholarship->criteria as $item)
                <li>
                  {{ $item->criteria_text }}
                  <span>
                    <form action="{{ route('criteria.delete', $item->criteriaID) }}#scholarship-{{ $scholarship->scholarshipID }}" method="POST" style="display:inline;">
                      @csrf @method('DELETE')
            ...
            <div class="add-relationship">
              <form action="{{ route('criteria.add', $scholarship->scholarshipID) }}#scholarship-{{ $scholarship->scholarshipID }}" method="POST">
                @csrf
          ...
          <div class="relationship-section">
            ...
            <ul class="relationship-list">
              @foreach ($scholarship->benefits as $benefit)
                <li>
                  ...
                  <span>
                    <form action="{{ route('benefit.delete', $benefit->benefitID) }}#scholarship-{{ $scholarship->scholarshipID }}" method="POST" style="display:inline;">
                      @csrf @method('DELETE')
                      <button class="rel-delete-btn">Delete</button>
                    </form>
                  </span>
            ...
            <div class="add-relationship">
              <form action="{{ route('benefit.add', $scholarship->scholarshipID) }}#scholarship-{{ $scholarship->scholarshipID }}" method="POST">
                @csrf
          ...
          <div class="relationship-section">
            ...
            <ul class="relationship-list">
              @foreach ($scholarship->partners as $partner)
                <li>
                  ...
                  <span>
                    <form action="{{ route('partner.delete', [$scholarship->scholarshipID, $partner->partnerID]) }}#scholarship-{{ $scholarship->scholarshipID }}"
            ...
            <div class="add-relationship">
              <form action="{{ route('partner.add', $scholarship->scholarshipID) }}#scholarship-{{ $scholarship->scholarshipID }}" method="POST">
                @csrf
      ...
      <div class="modal-backdrop" id="modal-{{ $scholarship->scholarshipID }}">
        <div class="modal-box">
          ...
          <form method="POST" action="{{ route('scholarships.update', $scholarship->scholarshipID) }}#modal-{{ $scholarship->scholarshipID }}">
            @csrf @method('PUT')
            <div class="form-group">
            ...
            <div class="form-group">
              <label for="funding-{{ $scholarship->scholarshipID }}">Funding Organization</label>
              <input type="text" id="funding-{{ $scholarship->scholarshipID }}"
                     name="funding_organization" value="{{ $scholarship->funding_organization }}" required>
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
              ...
              <textarea id="desc-{{ $scholarship->scholarshipID }}"
                        name="description" rows="3">{{ $scholarship->description }}</textarea>
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
                ...
                <select id="target-{{ $scholarship->scholarshipID }}" name="target_group" required>
                  <option value="Bachelor" {{ $scholarship->target_group=='Bachelor'?'selected':'' }}>Bachelor</option>
                  <option value="Master"   {{ $scholarship->target_group=='Master'  ?'selected':'' }}>Master</option>
                  <option value="PHD"      {{ $scholarship->target_group=='PHD'     ?'selected':'' }}>PHD</option>
                </select>
              </div>
              <div class="form-group">
