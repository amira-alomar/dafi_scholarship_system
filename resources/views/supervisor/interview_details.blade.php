            ...
            <div class="max-w-5xl mx-auto px-8 py-8 space-y-8">
                <header class="bg-white rounded-lg shadow-sm border border-gray-200 p-8">
                    <div class="flex items-center justify-between">
                        <div>
                            <h1 class="text-2xl font-semibold text-gray-900 mb-2">Interview Assessment</h1>
                                ...
                                @if ($interview)
                                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                                        <div class="space-y-6">
                                            ...
                                            <div>
                                                <label class="block text-sm font-medium text-gray-500 mb-2">Interview
                                                    Date</label>
                                                <p class="text-lg font-medium text-gray-900">
                                                    {{ date('F j, Y', strtotime($interview->interview_date)) }}</p>
                                            </div>
                                        </div>

                                        <div class="space-y-6">
                                            <div>
                                                <label class="block text-sm font-medium text-gray-500 mb-2">Performance
                                                    Level</label>
                                                <span
                                                    class="inline-flex px-3 py-1 text-sm font-medium rounded-full
                                                    {{ strtolower($interview->performance_level) === 'excellent'
                                                        ? 'bg-green-100 text-green-800 border border-green-200'
                                                        : (strtolower($interview->performance_level) === 'good'
                                                            ? 'bg-blue-100 text-blue-800 border border-blue-200'
                                                            : 'bg-yellow-100 text-yellow-800 border border-yellow-200') }}">
                                                    {{ ucfirst($interview->performance_level) }}
                                                </span>
                                            </div>

                                            <div>
                                                <label
                                                    class="block text-sm font-medium text-gray-500 mb-2">Recommendation</label>
                                                <span
                                                    class="inline-flex px-3 py-1 text-sm font-medium rounded-full
                                                    {{ strtolower($interview->recommendation) === 'recommend'
                                                        ? 'bg-green-100 text-green-800 border border-green-200'
                                                        : 'bg-red-100 text-red-800 border border-red-200' }}">
                                                    {{ ucfirst($interview->recommendation) }}
                                                </span>
                                            </div>
                                        </div>

                                        <div class="md:col-span-2">
                                            <label class="block text-sm font-medium text-gray-500 mb-3">Interview
                                                Notes</label>
                                            <div class="bg-gray-50 rounded-lg p-4 border border-gray-200">
                                                <p class="text-gray-900 leading-relaxed">
                                                    {{ $interview->notes ?? 'No additional notes provided.' }}</p>
                                            </div>
                                        </div>
                                    </div>
                                @else
                                    <div class="text-center py-12">
                                        <svg class="w-16 h-16 text-gray-300 mx-auto mb-4" fill="none"
                                            ...
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"
                                                d="M8 7V3a4 4 0 118 0v4m-4 12v-6m0-6h.01M6 20h12a2 2 0 002-2v-9a2 2 0 00-2-2H6a2 2 0 00-2 2v9a2 2 0 002 2z" />
                                        </svg>
                                        <h3 class="text-lg font-medium text-gray-900 mb-2">No Interview Conducted</h3>
                                        <p class="text-gray-600">Interview has not been scheduled or completed for this
                                            candidate.</p>
                                    </div>
                                @endif
                            </div>
                        ...
                        <section
                            ...
                            <div class="p-6">
                                ...
                                @if ($stageProgress->status === 'pending')
                                    <div class="space-y-3">
                                        <form action="{{ route('interview.accept', ['studentID' => $student->id]) }}"
                                            method="POST">
                                            @csrf
                                            <button type="submit"
                                        ...
                                        <form action="{{ route('interview.reject', ['studentID' => $student->id]) }}"
                                            method="POST">
                                            @csrf
                                            <button type="submit"
