...
<body class="flex min-h-screen font-poppins">
    ...
    <div class="container mx-auto px-4 py-6 lg:px-8">
        ...
        <!-- Success Message -->
        @if (session('success'))
            <div class="success-message mb-6 bg-green-50 border-l-4 border-green-400 p-4 rounded-r-lg shadow-md animate-fade-in">
        ...
        <div class="space-y-6">
            @foreach ($applications as $application)
                <div class="application-card bg-white rounded-2xl shadow-lg border border-gray-200 p-6 lg:p-8 hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1 animate-fade-in">
                    <!-- Applicant Header -->
                    <div class="flex items-center justify-between mb-6 pb-4 border-b border-gray-200">
                        <h2 class="text-2xl lg:text-3xl font-bold text-gray-800 flex items-center">
                            <div class="w-10 h-10 bg-blue-100 rounded-full flex items-center justify-center mr-3">
                                <span class="text-blue-600 font-bold text-lg">{{ substr($application->user->fname, 0, 1) }}</span>
                            </div>
                    ...
                    <div class="mb-6">
                        ...
                        <div class="grid gap-3 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
                            @foreach ($application->stageProgress as $progress)
                                <div class="progress-item bg-gray-50 rounded-xl p-4 border border-gray-200 hover:bg-gray-100 transition-colors duration-200">
                                    <div class="flex items-center justify-between mb-3">
                                        <span class="progress-stage font-semibold text-gray-800 capitalize">
                                            {{ $progress->stage->name }}
                                        </span>
                                        <span class="px-3 py-1 rounded-full text-xs font-medium
                                            @if(strtolower($progress->status) === 'passed') 
                                                bg-green-100 text-green-800
                    ...
                    <div class="mb-6">
                        @if ($application->notes)
                            <div class="notes-section bg-amber-50 border border-amber-200 rounded-xl p-4">
                                ...
                                <p class="notes-content text-amber-700 text-sm leading-relaxed">{{ $application->notes }}</p>
                            </div>
                        @else
                    ...
                    @if ($application->status == 'pending')
                        <div class="action-buttons flex flex-col sm:flex-row gap-3 pt-4 border-t border-gray-200">
                            <form action="{{ route('finalApplication.approve', ['applicationID' => $application->applicationID]) }}" 
                                  method="POST" class="flex-1">
                                @csrf
                                <button type="submit" 
                                        class="accept-btn w-full px-6 py-3 bg-green-600 hover:bg-green-700 text-white font-semibold rounded-xl transition-all duration-200 transform hover:scale-105 hover:shadow-lg flex items-center justify-center">
                            ...
                            <form action="{{ route('finalApplication.reject', ['applicationID' => $application->applicationID]) }}" 
                                  method="POST" class="flex-1">
                                @csrf
                                <button type="submit" 
                                        class="reject-btn w-full px-6 py-3 bg-red-600 hover:bg-red-700 text-white font-semibold rounded-xl transition-all duration-200 transform hover:scale-105 hover:shadow-lg flex items-center justify-center">
                                    <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
