    ...
    <div class="main-content" id="mainContent">
        @if (isset($message))
            <div class="card p-12 text-center max-w-2xl mx-auto">
                ...
                <h3 class="text-2xl font-bold text-gray-800 mb-3">Service Unavailable</h3>
        ...
        @else
            ...
            <div class="max-w-7xl mx-auto">
                ...
                <div class="mb-8">
                    <button id="toggleForm" class="btn-primary px-8 py-4 text-lg font-semibold inline-flex items-center space-x-3">
                    ...
                    <div class="card p-8 mb-8 bg-gradient-to-r from-yellow-50 to-orange-50 border-yellow-200">
                        <div class="flex items-center space-x-4">
                            ...
                            <div>
                                ...
                                <p class="text-yellow-700 mt-1">Candidates must pass the examination before scheduling interviews.</p>
                            </div>
                        </div>
                    </div>
                @else
                        ...
                        <div class="card p-8">
                            <h3 class="text-2xl font-bold text-gray-900 mb-6 flex items-center space-x-3">
                                <svg class="w-6 h-6 text-blue-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3a2 2 0 012-2h4a2 2 0 012 2v4m0 0V9a2 2 0 01-2 2H10a2 2 0 01-2-2V7m0 0a3 3 0 00-3 3v10a1 1 0 001 1h8a1 1 0 001-1V10a3 3 0 00-3-3z"></path>
                                </svg>
                                <span>Schedule Interview</span>
                            </h3>
                            
                            <form action="{{ route('interviewResult.store', ['scholarshipID' => $scholarshipID]) }}" method="POST" class="space-y-6">
                                @csrf
                                <div class="grid gap-6 md:grid-cols-3">
                                        ...
                                        <select name="student_id" required class="form-input w-full">
                                            ...
                                            @foreach ($eligible as $item)
                                                <option value="{{ $item->application->user->id }}">
                                                    ID: {{ $item->application->user->id }} — {{ $item->application->user->fname }}
                                                </option>
                                            @endforeach
                                        </select>
                                    </div>
                                    ...
                                    <div>
                                        ...
                                        <input type="date" name="interview_date" required class="form-input w-full" value="{{ now()->toDateString() }}" />
                                    </div>
                                ...
                                <div class="pt-4">
                                    <button type="submit" class="btn-primary px-8 py-3 inline-flex items-center space-x-2">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <div class="overflow-x-auto">
                        <table class="w-full">
                            <thead class="bg-gray-100 sticky top-0">
                                <tr>
                                    <th class="px-6 py-4 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Interview ID</th>
                                    <th class="px-6 py-4 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Student Name</th>
                                    <th class="px-6 py-4 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Interview Date</th>
                                    ...
                                    <tr class="table-row transition-all duration-200">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <span class="inline-flex px-3 py-1 text-xs font-semibold rounded-full capitalize
                                                @if($i->status === 'completed') bg-green-100 text-green-800
                                                @elseif($i->status === 'scheduled') bg-blue-100 text-blue-800
