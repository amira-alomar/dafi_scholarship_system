            ...
            @if (!$formClosed)
                <div class="flex items-center justify-center h-full p-8">
                    <div class="bg-amber-50 border border-amber-200 rounded-lg p-8 max-w-md text-center">
                        ...
                        <p class="text-amber-700">Examination stage cannot commence until all application procedures are completed.</p>
                    </div>
                </div>
            @else
                            ...
                            <div class="flex flex-col lg:flex-row lg:justify-between lg:items-center">
                                ...
                                <div class="mt-6 lg:mt-0">
                                    @if ($eligibleApplications->count() > 0)
                                        <form action="{{ route('supervisor.endExamStage', $scholarshipID) }}" method="POST" class="inline-block">
                                            @csrf
                                        ...
                                        <div class="p-6">
                                            <div class="flex justify-between items-start mb-4">
                                                <div class="flex-1">
                                                    ...
                                                    <p class="text-gray-600 text-sm mb-2">{{ $progress->application->user->email }}</p>
                                                    <p class="text-gray-500 text-xs">Application ID: {{ $progress->application->applicationID }}</p>
                                                </div>
                                                <span class="inline-flex px-3 py-1 text-xs font-medium rounded-full {{ $progress->application->stageProgress->isNotEmpty() ? 'bg-green-50 text-green-700 border border-green-200' : 'bg-yellow-50 text-yellow-700 border border-yellow-200' }}">
                                            ...
                                            <div class="flex flex-col sm:flex-row gap-3">
                                                @if ($progress->application->stageProgress->isNotEmpty())
                                                    <button disabled class="px-4 py-2.5 bg-gray-100 text-gray-500 text-sm font-medium rounded-md cursor-not-allowed flex-1">
                                                        Examination Scheduled
                                                    </button>
                                                @else
                                                    <div x-data="{ showModal: false }" class="flex-1">
                                                        <button @click="showModal = true" class="w-full px-4 py-2.5 bg-red-600 text-white text-sm font-medium rounded-md hover:bg-red-700 transition-colors">
                                                            Send Invitation
                                                        </button>

                                                        <div x-show="showModal" class="fixed inset-0 z-50 flex items-center justify-center modal-overlay" x-transition>
                                                            <div class="bg-white rounded-lg shadow-xl max-w-md w-full mx-4 p-6">
                                                                <h3 class="text-lg font-semibold text-gray-900 mb-6">Schedule Examination</h3>
                                                                <form action="{{ route('exam.sendInvitation', ['applicationID' => $progress->application->applicationID]) }}" method="POST" class="space-y-4">
                                                                    @csrf
                                                                    <div>
                                                                        <label class="block text-sm font-medium text-gray-700 mb-2">Exam Date & Time</label>
                                                                        <input type="datetime-local" name="exam_date" required class="w-full border border-gray-300 rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"/>
                                                                    </div>
                                                                    <div>
                                                                        <label class="block text-sm font-medium text-gray-700 mb-2">Subject</label>
                                                                        <input type="text" name="exam_subject" required class="w-full border border-gray-300 rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"/>
                                                                    </div>
                                                                    <div>
                                                                        <label class="block text-sm font-medium text-gray-700 mb-2">Additional Details <span class="text-gray-500">(Optional)</span></label>
                                                                        <textarea name="exam_details" rows="3" class="w-full border border-gray-300 rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500" placeholder="Special instructions or requirements..."></textarea>
                                                                    </div>
                                                                    <div class="flex justify-end gap-3 pt-4">
                                                                        <button type="button" @click="showModal = false" class="px-4 py-2 text-sm font-medium text-gray-700 bg-gray-100 rounded-md hover:bg-gray-200 transition-colors">
                                                                            Cancel
                                                                        </button>
                                                ...
                                                <a href="{{ route('exam.details', ['studentID' => $progress->application->idUser]) }}" class="px-4 py-2.5 bg-gray-900 text-white text-sm font-medium rounded-md hover:bg-gray-800 transition-colors text-center">
                                                    View Details
                                                </a>
                                            </div>
                                        </div>
                        ...
                        @else
                            <div class="text-center py-16">
                                <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-12 max-w-md mx-auto">
                                    ...
                                    <p class="text-gray-600">There are currently no students eligible for examination scheduling.</p>
                                </div>
                            </div>
                        @endif
                    @endif
