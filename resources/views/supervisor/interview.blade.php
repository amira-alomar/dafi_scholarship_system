                ...
                @if (!$examClosed)
                    <div class="bg-amber-50 border border-amber-200 rounded-lg p-6">
                        <div class="flex items-center">
                            ...
                            <p class="text-amber-800 font-medium">Interview stage cannot commence until all examination
                                ...
                                <div class="mt-6 lg:mt-0">
                                    @forelse ($eligibleApplications as $progress)
                                        <form action="{{ route('supervisor.endInterviewStage', $scholarshipID) }}"
                                            ...
                                            @csrf
                                            <button id="conclude-interview-btn" type="button"
                                    ...
                                    <div class="p-6">
                                        <div class="flex flex-col lg:flex-row lg:justify-between lg:items-center gap-6">
                                            <div class="flex-1">
                                                ...
                                                <p class="text-gray-600 mb-3">{{ $progress->application->user->email }}
                                                </p>
                                                <span
                                                    class="inline-flex px-3 py-1 text-xs font-medium rounded-full {{ $progress->application->stageProgress->first() ? 'bg-blue-50 text-blue-700 border border-blue-200' : 'bg-gray-50 text-gray-600 border border-gray-200' }}">
                                            ...
                                            <div class="flex flex-col sm:flex-row gap-3">
                                                @if ($progress->application->stageProgress->isNotEmpty())
                                                    <button disabled
                                                        class="px-5 py-2.5 bg-gray-100 text-gray-500 text-sm font-medium rounded-md cursor-not-allowed">
                                                        Interview Scheduled
                                                    </button>
                                                @else
                                                    <div x-data="{ open: false }">
                                                        <button @click="open = true"
                                                            class="px-5 py-2.5 bg-red-600 text-white text-sm font-medium rounded-md hover:bg-red-700 transition-colors">
                                                            Schedule Interview
                                                        </button>
                                                ...
                                                <a href="{{ route('interview.details', ['studentID' => $progress->application->idUser]) }}"
                                                    class="px-5 py-2.5 bg-gray-900 text-white text-sm font-medium rounded-md hover:bg-gray-800 transition-colors text-center">
                                                    View Details
                                                </a>
                                            </div>
                                        </div>
                            ...
                            @empty
                                <div class="text-center py-12">
                                    <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-8">
                                        ...
                                        <p class="text-gray-600">There are currently no students eligible for interview
                                            scheduling.</p>
                                    </div>
                                </div>
                            @endforelse
...
document.getElementById('conclude-interview-btn').addEventListener('click', function() {
  Swal.fire({
    title: 'Are you sure?',
    text: 'This will conclude the Interview stage for all eligible applicants!',
    icon: 'warning',
